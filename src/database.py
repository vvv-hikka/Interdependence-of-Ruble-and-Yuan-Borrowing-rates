...
try:
    from config import DB_PATH, DB_TABLES
except ImportError:
    DB_PATH = Path(__file__).parent.parent / "bond_rates_database.db"
    DB_TABLES = {}
...
class DatabaseManager:
    ...
    def create_combined_monthly_view(self) -> pd.DataFrame:
        """
        Create a combined view of all monthly data.
        """
        print("\nCreating combined monthly view...")
        
        # Get all tables
        tables = self.list_tables()
        tables = [t for t in tables if not t.startswith('_')]  # Exclude internal tables
